...
def kde_plot(df, bw, xmin, xmax, c, l, label = None):
    import matplotlib.pyplot as plt
    plt.xlim(xmin, xmax)
    
    
    kde = df.plot.kde(bw_method = bw, color = c, linestyle = l, 
                      label = label)
...
for i in range(n):
    ...
    if type(list_cols[i]) != list:
        ...
        for k in list_cols[i]:
            kde = kde_plot(df.iloc[:, k], bw, xmin, xmax, cs[n1], ls[n2])
            ax1 = plt.gca()
            ax1.spines['right'].set_visible(True)
            ax1.spines["top"].set_visible(True)
            n1 += 1
    ...
    if con == 'y':
        ...
        sizes, lb, coordinates = endmembers(coords, df_e, 1)
        m = 0
